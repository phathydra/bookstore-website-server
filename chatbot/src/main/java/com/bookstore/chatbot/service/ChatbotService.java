...
public class ChatbotService {
    ...
    private String fetchBooks(String genre, String author, String type) {
        try {
            // Construct the URL for the BookController's filter endpoint
            StringBuilder bookServiceUrl = new StringBuilder("http://localhost:8081/api/book/filter?");
            if (!author.isEmpty()) {
                bookServiceUrl.append("bookAuthor=").append(author);
            }
            if (!genre.isEmpty()) {
                if (!author.isEmpty()) bookServiceUrl.append("&");
                bookServiceUrl.append("mainCategory=").append(genre);
            }
            bookServiceUrl.append("&page=0&size=5");

            // Call the BookController's filter endpoint
            String response = bookServiceRestTemplate.getForObject(bookServiceUrl.toString(), String.class);
            ...
            StringBuilder formattedResponse = new StringBuilder(
                    type.equals("search")
                            ? "Dưới đây là kết quả tìm kiếm của quý khách:\n"
                            : "Dưới đây là những cuốn sách tôi đề xuất cho quý khách:\n"
            );
            for (JsonNode bookNode : contentArray) {
                ...
                formattedResponse.append("- ").append(bookLink).append(" của ").append(bookAuthor).append("\n");
            }
